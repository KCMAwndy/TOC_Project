...
var mapPathArray = ["map01.png", "map02.png", "map03.png", "map04.png", "map05.png"];
var charObj = [];
var efStatus = [];
var charColor = ["", "", "", "", "", ""];
var mapChoosen = "";
...
function setStateAndLeave(roomData) {
	localStorage.setItem("roomData", JSON.stringify(roomData))
	window.location.href = "/Web_Frontend/MVP_Build_5/home.html"
}
...
function exportJASON() {
	...
	var mapObject = {}
	mapObject["gameMap"] = { name: mapChoosen };
	mapObject["users"] = playerData
	var exportData = mapObject
	console.log(exportData)
	createRoom(exportData).then(data => {
		console.log("create room result", data)
		setStateAndLeave(data)
	});
}
...
function checkAll() {
	...
	if (charObj.length === 0 || charObj.length === 1) {
...
function sendCode() {
	...
	if (codeInput.value !== '') {
		getProgress(codeInput.value).then(data => {
